...
5. **Pick the window.** The "since you left" window starts at `state.last_run`, or 72 hours ago if no state. **Gap widening:** when a gap is detected — `state.streak == 0` with `state.last_run` set, i.e. the header reads `back after N days` — widen the window to cover the whole gap. The window start becomes the earlier of `state.last_run` and `now − N days`, where N is the detected gap (`days_since_last_run`). This matters when the user opens cafe on days they don't work: `last_run` can be only a day old while the real absence from the project is several days, and a one-day window would hide most of what they missed. All addon queries use this widened window for their `${state.last_run}` placeholder, so every "since you left" query covers the full time away.
