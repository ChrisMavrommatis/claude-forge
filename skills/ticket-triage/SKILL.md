---
name: ticket-triage
description: Disciplined triage of an incoming ticket — take the raw request, work out what it could mean, find and explain the real problem, and only then weigh solutions. Stops Claude jumping to solutions, inventing, or assuming. Keeps facts and guesses strictly apart; every fact is backed by a source. Read-only (writes only to `.plans`) until you explicitly say fix it. Use when you paste a Wrike / Jira / ADO ticket or say "I have this ticket". Examples — `/ticket-triage`, `/ticket-triage <name>` to resume.
---

# Ticket Triage

A ticket comes in. Instead of racing to a solution, this skill walks a fixed path:
take the raw words → work out what they could mean → find the real problem → explain
it → and **only then** weigh solutions. It holds that order, separates hard facts from
guesses, and never invents.

## What this is for

The failure this kills: you paste a ticket and Claude immediately invents solutions,
hallucinates, and makes assumptions — and you have to argue it back down. This skill
makes Claude slow down, stay on evidence, and ask before it leaps.

## When to use

- A Wrike / Jira / ADO ticket lands and you want to understand it before touching code.
- A vague or messy request where the real problem isn't obvious yet.
- Anything where you've been burned by Claude solving the wrong problem.

## When NOT to use

- You already know the exact fix and just want it done — skip triage, go fix it.
- A pure "how does X work?" question — that's exploration, not triage.
...
- **Bare + pasted ticket text** → start a NEW triage. Auto-fire also tends to happen
  when you say "I have this ticket" / paste a ticket, but the slash command is the
  reliable trigger.
...
## Guardrails — the heart of this skill

These override the usual urge to be helpful by guessing. Follow them exactly.
...
2. **Facts and guesses are different things — never blur them.**
...
4. **Don't fan out theories as if equal.** The failure mode is floating four guesses
   where one is right and three are noise. Prefer the possibility with evidence; keep
   the rest as ❓ Maybe, explicitly unconfirmed. Don't dress prediction as finding.
...
6. **Never act silently.** You may *offer* to explore ("want me to go out on a limb
   and check X?"), but you do not wander ahead on your own. The user steers.
...
8. **Read-only until told to fix.** You read code / system / logs to gather evidence,
   but you do not edit project code. You write only to `.plans`. The single exception:
   the user explicitly says "now fix that" (or similar) — then you may make the change.

## Facts vs. possibilities, and source tags

Every fact names where its evidence came from. More tags on one line = more
corroboration = stronger. Tags stack.
...
Keep every line to as few words as possible. Maybe / Open lines are phrased as
questions. Do **not** print RAW here — it lives in the file.
...
- **❓ Maybe** — possibilities as questions; never dressed as facts.
...
- **Written live, no asking** — every **claim** (a ❓) and every **piece of evidence**
  (a ✅ fact + its tag) the moment it appears. These are load-bearing; losing one means
  re-doing investigation. Refresh the summary block when they change.
...
- **Plain language always.** Short, clear English. Less is more if meaning isn't lost.
...
When there's no clear solution, or the problem turns out to be multi-layered — adding
debug instrumentation and running a discovery process to uncover the issue. That's a
