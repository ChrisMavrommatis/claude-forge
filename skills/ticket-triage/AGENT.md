...
Disciplined triage of an incoming ticket. The default Claude behaviour — paste a
ticket, immediately invent solutions, hallucinate, assume — is exactly what this kills.
The skill forces a fixed five-step path (RAW → MEANING → PROBLEM → EXPLAIN → SOLUTIONS),
gated step by step, with a hard stop before solutions.
...
- **Facts vs. possibilities is the spine.** A fact is confirmed and source-tagged; a
  possibility is a labelled guess. The whole skill exists to stop guesses masquerading
  as facts. Source tags: `#code #system #log #data #user #ticket` (last is weakest).
...
- **Five gated steps, hard stop before solutions.** The whole point is order. Soft
  gates everywhere except SOLUTIONS, which needs an explicit go. Claude may *offer* to
  explore but never wanders ahead silently.
...
- **Short, icon-led summary; RAW never shown in it.** The user wanted a two-second
  glance. RAW lives in the file only. Maybe / Open lines are phrased as questions.
...
- **Trail timing is a mix leaning live.** Claims and evidence are written the instant
  they appear (load-bearing — can't be lost). Fuller prose is flushed on pause/exit, on
  request, or on Claude's offer.
