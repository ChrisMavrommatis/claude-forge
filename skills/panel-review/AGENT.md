...
- **Two veto-eligible built-ins: `security` and `contract`.** `security` covers auth / secrets / injection / personal-data; `contract` covers the external surface but **vetoes narrowly** — a veto fires only on irreversible / destructive breaks (data-loss migrations). Recoverable external breaks (API shape, required fields, event payloads, versioning gaps) are `[BLOCK]`, not vetoes. (Settled with the maintainer; scope recorded in `personas/contract.md`.) QA was considered for veto too but rejected — QA's REJECT is usually about test-process gaps, not absolute defects; a HOLD is the right weight there.
